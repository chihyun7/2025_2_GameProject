...
public class DeliveryDriver : MonoBehaviour
{
    ...
    {
        //초기 상태  Event 발생
        driverEvents.OnMoveChanged?.Invoke(currentMoney);
    ...
    void HandleMovement()
    {
        ...
        if (moveDirection.magnitude > 0.1f)
        {
            ...
            ChangeBattery(-Time.deltaTime * 3.0f);      //이동 할 때마다 배터리 소모
        }
        else
        {
            if (isMoving)
    ...
    {
        currentMoney += amount;
        driverEvents.OnMoveChanged?.Invoke(currentMoney);       //돈 획득 후 이벤트 처리
    }
