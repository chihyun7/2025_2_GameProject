...
{
    [Header("던전 설정 ")]
    public int roomCount = 8;                   // 전체 생성하고 싶은 방 개수
    public int minSize = 4;                     // 방 최소 크기
    public int maxSize = 8;                     // 방최대 크기

    [Header("스포너설정 ")]
    public bool spawnEnemies = true;            //일반 방과 보스방에 적을 생성할지 여부
    public bool spawnTreasures = true;          // 보물 방에 보물을 생성항지 여부
    public int enemisPerRoom = 2;              //일반 방 1개당 생성할 적의 수

    private Dictionary<Vector2Int, Room> rooms = new Dictionary<Vector2Int, Room>();    //rooms: 방 중심 좌표 -> 방정보 매핑, 방 메타데-이터 보관
    private HashSet<Vector2Int> floors = new HashSet<Vector2Int>();                     //floors : 바닥 타일 좌표 집합 , 어떤칸이 바닥이닞 조회
    private HashSet<Vector2Int> walls = new HashSet<Vector2Int>();                      // walls : 벽타일 좌표 집합 , 바닥 주변을 자동으로 채운다
    ...
    void CreateRooms()
    {
        ...
        {
            var roomList = new List<Room>(rooms.Values);        // 이미 만들어진 방중 하나를 기준으로
            Room baseRoom = roomList[Random.Range(0, roomList.Count)];

            Vector2Int[] dirs =
            {
                Vector2Int.up * 6, Vector2Int.down * 6, Vector2Int.left * 6, Vector2Int.right * 6, //기준 방에서 상,하,좌,우 일정거리 세방후보
            };
            ...
            foreach (var dir in dirs)
            {
                ...
                if (AddRoom(newPos, newSize, type)) break;          // 방 영역이 기존 바닥과 겹치지 안흥면 추가성공 -> 다음방 생성으로 진행
            }
        }

        // 일반 방 중 일정 비율을 보물방으로 전환
    ...
    void CreateCorridor(Vector2Int start, Vector2Int end)
    {
        ...
        // ,y축 정렬 : x가 같아진뒤 start.y - > end y 로 한칸씩 이동
    ...
    void Render()
    {
        ...
        foreach (var pos in floors)
        {
            ...
            cube.transform.position = new Vector3(pos.x, 0, pos.y);         // ㅛ = 0 평면 배치
